...
namespace AsyncSocketServer
{
    public class AsyncSocketServer
    {
        ...
        private XYSocketProtocolMgr m_xySocketProtocolMgr;
        public XYSocketProtocolMgr XYSocketProtocolMgr { get { return m_xySocketProtocolMgr; } }
        ...
        public AsyncSocketServer(int numConnections)
        {
            ...
            m_xySocketProtocolMgr = new XYSocketProtocolMgr();
        }
        ...
        private void BuildingSocketInvokeElement(AsyncSocketUserToken userToken)
        {
            ...
            else if (flag == (byte)ProtocolFlag.XY)
                userToken.AsyncSocketInvokeElement = new XYSocketProtocol(this, userToken);
            if (userToken.AsyncSocketInvokeElement != null)
            {
                ...
                //{
                //        outputReceiver.AddNewLine(string.Format ("Building socket invoke element {0}.Local Address: {1}, Remote Address: {2}",
                //        userToken.AsyncSocketInvokeElement, userToken.ConnectSocket.LocalEndPoint, userToken.ConnectSocket.RemoteEndPoint));
        ...
        public void CloseClientSocket(AsyncSocketUserToken userToken)
        {
            ...
            try
            {
                ...
                //
                foreach (int userid in TCPServerControl.TCPServerControlInstance .SocketUserToken.Keys )
                {
                    if (userToken == TCPServerControl.TCPServerControlInstance.SocketUserToken[userid].AsyncSocketUserToken)
                    {
                        TCPServerControl.TCPServerControlInstance.LogOut(userid,false );
                        break;
                    }
                }
                if (outputReceiver != null)
                {
                    outputReceiver.AddNewLine("SocketShutdown.Both");
                }
            }
            catch (Exception E)
            {
                ...
                if (outputReceiver != null)
                {
                    outputReceiver.AddNewLine(string.Format("CloseClientSocket Disconnect client {0} error, message: {1}", socketInfo, E.Message));
                }
            }
